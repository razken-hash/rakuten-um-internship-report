\chapter{Visual Regression Testing Infrastructure}
...
\section{Storybook Stories Screenshot Preparation}
...
\subsection{Automated Screenshot Generation}
...
\subsection{Storybook-Related Commands}
...
\subsection{Screenshot Generation}
...
\section{Reg-Suit Configuration}
...
\subsection{Dependencies and Plugins}
...
\subsection{Reg-Suit Configuration File}
...
\section{Visual Regression Testing in CI}
...
Implementing a reliable \ac{VRT} system involves several challenges that can introduce false positives or unnecessarily increase the execution time and storage requirements of the testing pipeline. The main challenges encountered during the implementation are described below.

\subsection{Animations and Transitions}

CSS animations and transitions can introduce visual differences between two screenshots even when the underlying \ac{UI} has not changed. Since the exact state captured by the screenshot may depend on the timing of the animation, consecutive test runs can produce slightly different images.

To ensure deterministic screenshots, animations and transitions are disabled in the test environment. A dedicated CSS rule can be injected before capturing the screenshots:
...
This ensures that elements are captured in a stable state and prevents timing-dependent differences from being incorrectly reported as visual regressions.

\subsection{Dynamic and Non-Deterministic Data}

Some components frequently contain dynamic data that changes between executions, such as randomly generated identifiers, timestamps, dates, counters, or other runtime-generated values. If such values are rendered directly in the \ac{UI}, they can cause screenshots to differ even when no visual change has been introduced.

Whenever possible, these values are replaced with deterministic mock data in the test environment. For example, instead of generating a random identifier:

\begin{lstlisting}[language=typescript]
const id = crypto.randomUUID();
\end{lstlisting}

the test environment can provide a fixed value:

\begin{lstlisting}[language=typescript]
const id = "test-user-001";
\end{lstlisting}

The same principle applies to dates and timestamps, which can be fixed to a known value during testing. This guarantees that repeated executions produce the same rendered output.

Some elements, however, cannot be easily mocked or are not relevant to visual comparison. For these cases, a dedicated \texttt{no-screenshot} CSS class is used to exclude the corresponding elements from screenshots:
...
For example, this can be applied to third-party elements such as cookie banners, chat widgets, or other external components whose content is outside the scope of the visual regression tests:

\begin{lstlisting}[language=html]
<div class="no-screenshot">
    <!-- Dynamic or irrelevant content -->
</div>
\end{lstlisting}

This approach prevents irrelevant or inherently dynamic elements from generating false-positive visual differences.

\subsection{Large and High-Volume Images}

Images can significantly affect both the execution time and the size of the generated visual regression artifacts. Pages containing a large number of high-resolution images may require additional time for all assets to load before a screenshot can be captured. The resulting screenshots are also larger, increasing upload and download times as well as cloud storage consumption.

To ensure that screenshots are captured only after the required images have been loaded, the test environment waits for all relevant images to complete loading before taking the screenshot. For example:

\begin{lstlisting}[language=TypeScript]
await page.waitForLoadState('networkidle');
\end{lstlisting}

However, not every image contributes meaningful information to the visual comparison. For example, product images in a product listing may be numerous and large while the visual regression test is primarily concerned with the layout, typography, and structure of the page. Such images can therefore be hidden using the same \texttt{no-screenshot} mechanism:
...
This optimization has several benefits: it reduces screenshot generation time, decreases the size of the generated artifacts, reduces upload and download times during the \ac{CI} pipeline, and lowers the amount of storage required in \ac{GCS}.

\subsection{Deterministic Rendering}

Overall, the reliability of \ac{VRT} depends on ensuring that the same application state produces the same visual output across different executions. The testing environment therefore needs to control the main sources of non-determinism, including animations, dynamic data, external components, and asynchronous asset loading.

By disabling animations and transitions, replacing dynamic values with deterministic mock data, excluding irrelevant elements, and waiting for required assets to finish loading, the workflow produces stable screenshots that make genuine visual regressions easier to identify while minimizing false positives and unnecessary resource consumption.
