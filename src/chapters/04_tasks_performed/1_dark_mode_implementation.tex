...
\subsubsection{Dark Mode Definition, Importance, and Challenges}
...
\subsubsection{Syncing MUI theme with global CSS variables}
...
\subsubsection{Resolving the Flash Of Unstyled Content Problems}
...
\subsubsection{POC Implementation with MUI 5 and MUI 7}
...
\subsection{Phase 02: Technical Implementation in \texttt{next-common}}

\subsubsection{Migration to MUI v7}
...
\subsubsection{Setting \textbf{global-css} as the source of truth}
...
\subsubsection{{ThemeContextProvider}}
...
\subsubsection{\texttt{globalTheme} with CSS variables}
...
\subsubsection{Storybook integration}
...
\subsubsection{Header \& Footer adaptation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.9\textwidth]{../assets/dark_mode/header_light.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.9\textwidth]{../assets/dark_mode/header_dark.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.9\textwidth]{../assets/dark_mode/footer_light.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.9\textwidth]{../assets/dark_mode/footer_dark.png}
    \caption{Footer component in dark mode from Storybook.}
    \label{fig:footer-dark}
\end{figure}

\subsubsection{Circular Dependency Resolution}
...
\subsubsection{Adding dark mode to the rest of the ecosystem}
...
\subsection{Phase 03: Rollout to \texttt{fo-apps}}
...
\subsubsection{Application migration steps}

Each downstream application consuming shared packages such as \texttt{@rakutenfrance/mui}, \texttt{@rakutenfrance/header}, and \texttt{@rakutenfrance/footer} follows the same integration process:
...
\begin{enumerate}
    ...
    \item \textbf{Update theme-related imports:} Existing imports are adapted to follow the MUI v7 API conventions, especially for theme utilities such as \lstinline|Theme|, \lstinline|styled|, and \lstinline|useTheme|.

    \item \textbf{Integrate the shared theme provider:} Applications are wrapped with the existing \lstinline|MUIProvider| or \lstinline|AppRouterMUIProvider|, which internally uses \lstinline|ThemeContextProvider| to manage theme state, persistence, and synchronization.

    \item \textbf{Initialize the theme before rendering:} The \lstinline|ThemeInitScript| is injected into the application document to ensure that the correct theme is applied before the first render and prevent FOUC issues.
...
\subsection{Follow-ups and known limitations}

Although the dark mode foundation is fully implemented, some additional improvements and considerations remain:

\begin{itemize}
    \item \textbf{CSS variable transparency handling:} MUI's \lstinline|alpha()| utility cannot directly process CSS variables. Components requiring transparency rely on predefined overlay tokens instead.

    \item \textbf{Asset-specific adaptations:} Some visual assets may require dedicated handling when their original design does not support both themes.

    \item \textbf{Progressive adoption:} Existing applications must migrate progressively by updating dependencies, replacing hardcoded colors, and integrating the shared theme provider.

    \item \textbf{Visual regression monitoring:} Storybook and Chromatic remain the main tools for detecting visual regressions and ensuring theme consistency during future developments.
\end{itemize}
